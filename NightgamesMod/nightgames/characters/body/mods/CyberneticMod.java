...
public class CyberneticMod extends PartMod {
    ...
    public double applyBonuses(Combat c, Character self, Character opponent, BodyPart part, BodyPart target, double damage) {
        ...
        if (c.getStance().isPartFuckingPartInserted(c, opponent, target, self, part)) {
            if (target.moddedPartCountsAs(CockMod.enlightened)) {
                ...
                if (Global.random(3) == 0 || target.moddedPartCountsAs(CockMod.incubus)) {
                    String prefix = target.moddedPartCountsAs(CockMod.incubus) ? "Eager to gain a sample of "
                    ...
                    bonus += 15;
                    if (target.moddedPartCountsAs(CockMod.incubus) || Global.random(4) == 0) {
    ...
    public int counterValue(BodyPart part, BodyPart otherPart, Character self, Character other) { 
        return otherPart.moddedPartCountsAs(CockMod.incubus) ? 1 : otherPart.moddedPartCountsAs(
            CockMod.enlightened) ? -1 : 0;
    }
