...
public class Combat {
    ...
    private interface FastSkippablePhase extends Phase {}

    private interface SkippablePhase extends FastSkippablePhase {}
    ...
    private static class SkillSelectionPhase implements Phase {
        ...
        public boolean turn(Combat c) {
            if (c.p1act == null) {
                return c.p1.act(c, c.p2);
            } else if (c.p2act == null) {
                return c.p2.act(c, c.p1);
            } else {
            ...
            if (c.otherCombatants.size() > 0) {
                if (!Global.checkFlag("NoPetBattles")) {
                    for (var pet : c.otherCombatants) {
                        if (alreadyBattled.contains(pet.getCharacter())) {
                            continue;
                        }
                        for (var otherPet : c.otherCombatants) {
                            if (alreadyBattled.contains(otherPet.getCharacter())) {
                                continue;
                            }
                            if (!pet.getCharacter().getSelf().owner().equals(otherPet.getCharacter().getSelf().owner())
                                    && Global.random(2) == 0) {
    ...
    private void fetishDisadvantageAura(Character character, List<Character> opponents, String fetishType, ClothingSlot clothingType) {
        ...
        otherWithAura.ifPresent(other -> {
            Clothing clothes = other.getOutfit().getTopOfSlot(clothingType);
            boolean seeFetish = clothes == null || clothes.getLayer() <= 1 || other.getOutfit().getExposure() >= .5;
            var partDescription = other.body.getRandom(fetishType).describe(other);
            if (seeFetish && Global.random(5) == 0) {
                if (character.human()) {
                    write(character, "You can't help thinking about " + otherWithAura.get().nameOrPossessivePronoun() + " " + partDescription + ".");
                }
