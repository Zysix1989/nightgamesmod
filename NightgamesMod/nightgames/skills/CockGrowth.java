        ...
        if (res != Result.miss) {
            ...
            if (permanent) {
                if (part != null) {
                    ...
                    target.body.addReplace(new CockPart().newWithSize(SizeMod.COCK_SIZE_SMALL), 1);
                }
            } else {
                if (part != null) {
                    target.body.temporaryAddOrReplacePartWithType(part.upgrade().upgrade().upgrade().upgrade().upgrade(), 10);
                } else {
                    target.body.temporaryAddPart(new CockPart().newWithSize(SizeMod.COCK_SIZE_SMALL), 10);
                }
            }
        }
