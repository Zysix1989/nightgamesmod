    ...
    public boolean resolve(Combat c, Character target) {
        ...
        getSelf().addTemporaryTrait(Trait.ImitatedStrength, 999);
            ...
            getSelf().addTemporaryTrait(Trait.energydrain, 999);
                ...
                ((GenericBodyPart) getSelf().body.getRandomFeet()).addTemporaryMod(new DemonicMod(), 999);
                ((GenericBodyPart) getSelf().body.getRandomMouth()).addTemporaryMod(new DemonicMod(), 999);
        getSelf().addTemporaryTrait(Trait.succubus, 999);
        getSelf().addTemporaryTrait(Trait.soulsucker, 999);
        getSelf().addTemporaryTrait(Trait.energydrain, 999);
        getSelf().addTemporaryTrait(Trait.spiritphage, 999);
        ...
        getSelf().body.temporaryAddPart(wings, 999);
        wings.addTemporaryMod(new DemonicWingsMod(), 999);
        wings.addTemporaryMod(new SlimeWingsMod(), 999);
        var tail = (TailPart) getSelf().body.getRandom(TailPart.TYPE);
        if ( tail == null) {
            tail = new TailPart();
        }
        getSelf().body.temporaryAddPart(tail, 999);
        tail.addTemporaryMod(new DemonicTailMod(), 999);
        tail.addTemporaryMod(new SlimeTailMod(), 999);
        tail.addTemporaryMod(new PointedEarsMod(), 999);
